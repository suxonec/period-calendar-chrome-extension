...
- [content/content.js](content/content.js) — everything else: builds the closed shadow-DOM host, renders the pill and the expanded panel, owns all UI state (`year`, `notes`, `open`, `dismissed`, `editingIso`), wires events, and talks to `chrome.storage.local`.
